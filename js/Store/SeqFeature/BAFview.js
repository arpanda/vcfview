    ...
    async _readChunk(query) {
      ...
      await this.indexedData.getLines(
        ...
        (line, fileOffset) => {
          ...
          if (!bins[featureBin]) {
            bins[featureBin] = { score: 0, count: 0, lh_score:[], min_score: 0 };
          }
          ...
          const ad_score = fields[9 + this.sample].split(":")[AD].split(',').map(x=>+x);
          const gt_score = fields[9 + this.sample].split(":")[GT].split('/').map(x=>+x);
          const ad_a = ad_score[0];
          const ad_b = ad_score[1];
          
          
          if((gt_score[0] == 0 && gt_score[1] == 1) || (gt_score[0] == 1 && gt_score[1] == 0)){
            if(bins[featureBin].lh_score.length == 0 ){
              bins[featureBin].lh_score = linspace(0, 1, 200).map((value, index) => {
              ...
              bins[featureBin].lh_score = linspace(0, 1, 200).map((value, index) => {
                var lh_value = bins[featureBin].lh_score[index] * beta(ad_a, ad_b, value);
                sum = sum + lh_value;
                return lh_value
        ...
        (feature) => {
          ...
          field_list.forEach((val) => {
            if (typeof sample_name[val] != "undefined") {
              sample_score = sample_name[val].values[1] / (sample_name[val].values[0] + sample_name[val].values[1]);
            }
          });
          ...
          let sample_data = new SimpleFeature({
            ...
            data: {
              ...
              score: sample_score,
              source: samples[sample_position] + '_BAF'
            },
          });
