  ...
  export class Partition{
      ...
      this.bin_bands.forEach((bin_band, bin_band_index) => {
          ...
          for(var i=1; i<border.length; i++){
              ...
              if ((n_right <= 15) || (n_left <= 15) || n<=15){
                  ...
                  var ttest_2sample_1 = t_test_2_samples(seg_array.mean, seg_array.std, seg_array.data.length, seg_left_1.mean, seg_left_1.std, seg_left_1.data.length);
                  ...
                  var ttest_2sample_2 = t_test_2_samples(seg_array.mean, seg_array.std, seg_array.data.length, seg_right_1.mean, seg_right_1.std, seg_right_1.data.length);
    ...
    cnv_calling(){
      ...
      while(b < levels.length){
          ...
          var be = b; 
          if(be > bs + 1){
              var adj = adjustToEvalue(this.mean, this.std, this.rd, bs, be, 0.05 * bin_size / normal_genome_size);
              if(adj){
                  var bs, be = adj
          ...
          be = b
          if(be > bs + 1){
              adj = adjustToEvalue(this.mean, this.std, this.rd, bs, be, 0.05 * bin_size / normal_genome_size);
              if(adj){
                  bs, be = adj
                  segments.push([bs, be, +1])
                  // flags[bs:be] = ["A"] * (be - bs)
      ...
      while (b < levels.length){
          ...
          be = b
          if (be > bs + 1){
              if(gaussianEValue(this.mean, this.std, this.rd, bs, be) < 0.05 / normal_genome_size) {
                  segments.push([bs, be, -1])
