    ...
    return declare(VCFTabix, {
      ...
      async _readChunk(query) {
        ...
        if (globalCache.has(`${this.fileBlob}`)) {
          ...
          const { mean, sd, gcRD} = await this.getAvgAndSD(this.fileBlob, this.sample);
        ...
        for(let k=0; k < bins.length/ binFactor; k++){
          //console.log(k);
          if (!avgbin[k]) { 
            avgbin[k] = { bin_score: 0, gc_corrected:0, corrected_mean:0, call_score: 0 };
          }
          
          for(var j=k*10;j< 10*k + 10; j++){
            if(bins[j]){
              const featureBin = Math.max(Math.floor(bins[j].start / (binSize*binFactor)), 0);
        ...
        bins.forEach(sample => {
          ...
          const featureBin = Math.max(Math.floor(start / (binSize * binFactor)), 0);
      ...
      async _getFeatures( query, featureCallback, finishedCallback, errorCallback, ) {
        try {
              ...
              featureCallback(
                new SimpleFeature({
                  data: Object.assign(Object.create(feature), {
                    ...
                    source: sample.source + "_meanshift",
                  }),
                }),
              ),
              featureCallback(
      ...
      async getAvgAndSD(blob, sample) {
        ...
        for ( const chr in avgbin){
          let chr_score = [];
          for (let bin in avgbin[chr]){
            chr_score.push(avgbin[chr][bin].bin_score);
          }
          const chr_mean_rd = getMean(chr_score);
          // const chr_mean_sd = getSD(chr_score);
          const max_rd = parseInt(10 * chr_mean_rd +1);
          const rd_bin_size = parseInt(max_rd/10000);
  
          // console.log(max_bin);
          //console.log('check point 1');
          //console.log('max_value', parseInt(max_bin/rd_bin_size)*rd_bin_size+rd_bin_size);
          const max_value = parseInt(max_rd/rd_bin_size )* rd_bin_size + rd_bin_size;
          // console.log('mean_chr_rd: ',chr_mean_rd, 'mean_chr_sd: ',chr_mean_sd, ', max_rd: ', max_rd, ', max_value: ', max_value);
          const range_data = range_function(0, max_value, step = rd_bin_size);
          const dist_p = histogram(avgbin[chr], range_data);
          
          // console.log('chr name is : ', chr);
          // console.log('data_hist', dist_p);
          let fit_data = fit_normal(range_data, dist_p);
          console.log(fit_data);
          //console.log(hist_bins, hist_bins.count);
